...
def get_cos_similar_multi(v1: list, v2: list, top_k):
    ...
    neg_cos_sim = np.negative(cos_sim)
    index_top_k = np.argpartition(neg_cos_sim, kth=top_k)[:top_k]
    cos_sim_top_k = [cos_sim[i] for i in index_top_k]
    result = dict(zip(index_top_k, cos_sim_top_k))
...
def get_cos_similar_matrix(v1, v2, top_k):
    ...
    for i in range(len(cos_sim_matrix)):
        neg_cos_sim = np.negative(cos_sim_matrix[i])
        index_top_k = np.argpartition(neg_cos_sim, kth=top_k)[:top_k]
        cos_sim_top_k = [cos_sim_matrix[i][j] for j in index_top_k]
        dict_cos_sim = dict(zip(index_top_k, cos_sim_top_k))
        sorted_cos_sim = sorted(dict_cos_sim.items(), key=lambda x: x[1], reverse=True)
        result.append(sorted_cos_sim)
    return result
