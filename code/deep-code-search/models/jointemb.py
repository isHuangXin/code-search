...
class JointEmbeder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.name_encoder = SeqEncoder(config['n_words'], config['emb_size'], config['lstm_dims'])  # (10000, 512, 1024)
        self.api_encoder = SeqEncoder(config['n_words'], config['emb_size'], config['lstm_dims'])
        self.tok_encoder = BOWEncoder(config['n_words'], config['emb_size'], config['n_hidden'])  # (10000, 512, 512)
    ...
    def forward(self, name, name_len, apiseq, api_len, tokens, tok_len, desc_anchor, desc_anchor_len, desc_neg,
                desc_neg_len):
