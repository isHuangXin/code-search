...
class SeqEncoder(nn.Module):
    ...
    def init_weights(self):
        ...
        nn.init.uniform_(self.embedding.weight, -0.1, 0.1)
        """
        常数初始化, 使值为常数val nn.init.constant_(w, 0.3)
        torch.nn.init.constant_(tensor, val)
        """
        nn.init.constant_(self.embedding.weight[0], 0)
        print("for name, param in self.lstm.named_parameters():")
    ...
    def forward(self, inputs, input_lens=None):
        ...
        if input_lens is not None:  # reorder and pad
            ...
            hids, lens = pad_packed_sequence(hids,
                                             batch_first=True)  # hids:[batch_size x seq_len x (n_dir*hid_sz)](biRNN)
