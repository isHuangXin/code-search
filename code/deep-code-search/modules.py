...
class BOWEncoder(nn.Module):
    ...
    def forward(self, input, input_len=None):
        ...
        " input: [batch_sz x seq_len x 1] "
        " embedded: [batch_sz x seq_len x emb_sz] "
        embedded = self.embedding(input)
        " [batch_size x seq_len x emb_size] "
        embedded = F.dropout(embedded, 0.25, self.training)

        # try to use weighting scheme to summarize bag of word embeddings
        ...
        # word_weights = self.word_weights(input)
        # embedded = word_weights * embedded

        # max pooling word vectors
        maxpooling = nn.MaxPool1d(kernel_size=seq_len, stride=seq_len)
        " [batch_size x emb_size] "
        output_pool = maxpooling(embedded.transpose(1, 2)).sequeeze(2)
        " torch.tanh(output_pool) "
...
class SeqEncoder(nn.Module):
    ...
    def forward(self, inputs, input_lens=None):
        ...
        """
        batch_size, sqe_len = inputs.size()
        " input: [batch_sz x seq_len]"
        " embedded: [batch_sz x seq_len x emb_sz] "
        ...
        if input_lens is not None:
            ...
            input_sorted = inputs.index_select(0, indices)
            inputs = pack_padded_sequence(input_sorted, input_lens_sorted.data.tolist(), batch_first=True)
        hids, (h_n, c_n) = self.lstm(inputs)

        if input_lens is not None:
            # reorder and pad
            _, inv_indices = indices.sort()
            " hids: [batch_size x seq_len x (n_dir * hid_sz)](biRNN) "
            hids, lens = pad_packed_sequence(hids, batch_first=True)
            hids = hids.index_select(0, inv_indices)
            h_n = h_n.index_select(1, inv_indices)
        " [n_layers x n_dirs x batch_sz x hid_sz] "
        h_n = h_n.view(self.n_layers, 2, batch_size)
        " get the last layer [n_dirs x batch_sz x hid_sz] "
        h_n = h_n[-1]

        # commenting th followling line significantly improves the performance
...
def get_cosine_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps, num_cycles=.5, last_epoch=-1):
    ...
    """
    def lr_lambda(currend_step):
        if currend_step < num_warmup_steps:
            return float(currend_step) / float(max(1, num_warmup_steps))
        progress = float(currend_step - num_warmup_steps) / float(max(1, num_training_steps - num_warmup_steps))
        return max(0., 0.5 * (1.+math.cos(math.pi * float(num_cycles) * 2. * progress)))
    return LambdaLR(optimizer, lr_lambda, last_epoch)

def get_word_weights(vocab_size, padding_idx=0):
    """
    construct a word weighting table
    """
    def cal_weight(word_idx):
        return 1-math.exp(-word_idx)
    weight_table = np.array([cal_weight(w) for w in range(vocab_size)])
    if padding_idx is not None:
        weight_table[padding_idx] = 0.  # zero vector fo padding dimension
