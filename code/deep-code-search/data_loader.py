...
class CodeSearchDataset(data.Dataset):
    ...
    def __init__(self, data_dir, f_name, max_name_len, f_api, max_api_len,
        ...
        """
        ('/home/wanyao/huangxin/data/deep_code_search/github/',
         'train.name.h5', 6, 'train.apiseq.h5', 30, 'train.tokens.h5', 50, 'train.desc.h5', 30)
        """
        ...
        dcs-data/:
            for train dataset:
                - self.idx_names.shape: (18123872,)
                - self.idx_apis.shape: (18123872,)
                - self.idx_tokens.shape: (18123872,)
                - self.idx_descs.shape: (18123872,)
            for valid dataset:
                - self.idx_names.shape: (100000,)
                - self.idx_apis.shape: (100000,)
                - self.idx_tokens.shape: (100000,)
                - self.idx_descs.shape: (100000,)
        """
        """
        dcs-data/:
            for use dataset:
                - self.idx_names.shape: (16262602,) 
                - self.idx_apis.shape: (16262602,)
                - self.idx_tokens.shape: (16262602,)
                - self.idx_descs.shape: None
        """
...
if __name__ == '__main__':
    ...
    train_set = CodeSearchDataset(input_dir, 'train.name.h5', 6, 'train.apiseq.h5', 20, 'train.tokens.h5', 30,
                                  'train.desc.h5', 30)
    train_data_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=1, shuffle=False, num_workers=1)
    valid_set = CodeSearchDataset(input_dir, 'valid.name.h5', 6, 'valid.apiseq.h5', 20, 'valid.tokens.h5', 30,
                                  'valid.desc.h5', 30)
