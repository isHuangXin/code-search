...
def train(args):
    ...
    os.makedirs(f'./output/{args.model}/tmp_results', exist_ok=True)
    ...
    tb_writer = SummaryWriter(f"./output/{args.model}/{args.dataset}/logs/") if args.visual else None
    ...
    def save_model(model, ckpt_path, ckpt_path_pth):
    ...
    global global_step
    global_step = args.reload_from + 1  # global_step initialize from 0
    for epoch in range(int(args.reload_from / n_iters) + 1, config['nb_epoch'] + 1):
        itr_start_time = time.time()
        losses = []
        for batch in data_loader:
            # batch = {list: 10}
            # batch里的每一个tensor分别对应下面的每一项
            # name, name_len :                      name为code方法名, name_len==6
            # apiseq, api_len:                      apiseq为code api名, apiseq==30
            # tokens, tok_len:                      tokens为code tokens, tok_len==50
            # desc_anchor, desc_anchor_len:         desc_anchor为code的正向description, desc_anchor_len==30
            # desc_neg, desc_neg_len:               desc_neg为code的负向description, desc_neg_len==30
            model.train()
            ...
            if config['fp16']:
                ...
                if global_step % args.save_every == 0:
                    if args.dataset == '':
                        folder = f'/home/wanyao/huangxin/model/dcs-model/{args.model}/dcs-data'
                    else:
                        folder = f'/home/wanyao/huangxin/model/dcs-model/{args.model}/{args.dataset}'

                    if not os.path.exists(folder):
                        os.makedirs(folder)

                    ckpt_path = f'{folder}/step{global_step}.h5'
                    ckpt_path_pth = f'{folder}/step{global_step}.pth'
                    save_model(model, ckpt_path, ckpt_path_pth)
                    if IS_ON_NSML:
                        nsml.save(ckeckpont=f'model_step{global_step}')
...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='/home/wanyao/huangxin/data/dcs-data',
    ...
    # Training Arguments
    parser.add_argument('--log_every', type=int, default=100, help='interval to txt_file autoencoder training results')
    parser.add_argument('--valid_every', type=int, default=50000, help='interval to validation')
